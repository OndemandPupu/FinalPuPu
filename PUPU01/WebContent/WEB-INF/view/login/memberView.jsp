				...
				<div class="row">
					<div class="col-sm-5 form-box">
						...
						<div class="form-group has-success has-feedback" >
							<form action="/member/join" method="post" class="registration-form">
								<p class="form-group">
								...
								<button type="button" class="buttonT button1" id="join">
								<a style="color: white;">회원가입</a></button>
								<button type="reset" class="buttonT button1" >
								<a style="color: white;">다시작성</a></button>
...
if(v==""){
	document.getElementById("rst").innerHTML = "<p class='text-warning' style='color: #BDBDBD'>아이디를 입력해 주세요</p>";
	return;
}
		...
		xhr.onreadystatechange = function() {
			if (xhr.status == 200 & xhr.readyState == 4) {
				...
				if (t == "TRUE" ) {
					...
					html = "<p class='text-warning'>이미 사용중인 아이디 입니다.</p>";
				}else if(t=="FALSE"){
					html = "<p class='text-primary'>사용가능한 아이디 입니다.</p>";
				}else {
					html = "<p  class='text-warning'>아이디 입력 필수</p>";
...
<script>
	document.getElementById("pass2").addEventListener("blur", function() {
		...
		var html;
		if (pass != pass2) {
			html = "<p class='text-warning'>패스워드가 일치하지 않습니다.</P>";
		} else {
			html = html = "<p class='text-primary'>패스워드가 일치 합니다.</p>";;
		}
			...
			.addEventListener(
					...
					function() {
						...
						if (reg.test($("#emailId").val())) {
							...
							xhr.open("get", "/member/joinAjax2?email=" + v,
									true);
							xhr.onreadystatechange = function() {
								if (xhr.status == 200 & xhr.readyState == 4) {
									var t = xhr.responseText;
												...
												.addEventListener(
														...
														function() {
															...
															if (t == v1) {
																...
																window
																		.alert("정상인증처리되지않았습니다.인증코드를 다시 확인해주세요.");
															}
														});
									} else {
